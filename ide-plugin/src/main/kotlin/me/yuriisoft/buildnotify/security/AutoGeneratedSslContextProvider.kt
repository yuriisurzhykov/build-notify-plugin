 ...
 *
 * Phase 2 change: [CertificateBundle.keyManagers] is now populated so that
 * [MtlsWebSocketServerFactory] can build a composite [SSLContext] that combines
 * the server key with a client [javax.net.ssl.TrustManager].
 */
class AutoGeneratedSslContextProvider : SslContextProvider {
    ...
    private fun loadExisting(): CertificateBundle {
        ...
        return CertificateBundle(
            ...
            keyManagers = keyManagers,         // ← Phase 2: expose for mTLS composition
        )
    }
    ...
    private companion object {
        ...
        // Phase 2: SSLContext is now built from a pre-extracted Array<KeyManager>
        // instead of directly from KMF, so the same array can be reused when
        // building the mTLS composite context in MtlsWebSocketServerFactory.
